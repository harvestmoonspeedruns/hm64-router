function get_actions_elli_photo (d = 3, g = 300, is_sunny = 1) {
	...
	if (d > 31 && d <= 69) { // Summer to Photo
		if (flags['chicken_funeral'] == 1) {
		...
		} else if (is_sunny == 1 && ["MON", "TUES", "THURS", "FRI"].includes(get_day_of_week(d, true)) && d != 39 && d != 54 && d != 64) {
			...
			// Elli
			a.push({'desc':("Talk" + ((get_day_of_week(d, true) == "MON") ? (" (" + elli_mon_loc + ")") : "")), 'cid':elli_id, 'val':1, 'sel':(!flags['new_mus_box']), 't2':"MusBox"});
			a.push({'desc':"MusBox", 'cid':elli_id, 'val':6, 'sel':flags['new_mus_box'], 'sel':(aff[rick_id] >= _RICK_FIX_MIN - 3), 'sr':true, 't2':a[a.length - 1]['desc']});
			
			var elli_gift = (vars['chickens'] == 0) ? "M/L Fish" : "Egg";
			a.push({
				'desc':("Elli-" + elli_gift + ((get_day_of_week(d, true) == "MON") ? (" (" + elli_mon_loc + ")") : "")),
				'cid':((vars['chickens'] == 0 || flags['recipe_elli'] == 1) ? elli_id : [elli_id, "f_recipe_elli"]),
				'val':((vars['chickens'] == 0) ? 3 : ((flags['recipe_elli'] == 1) ? 4 : [6, 1])), 'sr':true,
				'sel':(vars['chickens'] > 0),
				't2':("Elli-gift" + ((get_day_of_week(d, true) == "MON") ? (" (" + elli_mon_loc + ")") : ""))
			});
			a.push({
				'desc':("Elli-gift" + ((get_day_of_week(d, true) == "MON") ? (" (" + elli_mon_loc + ")") : "")),
				'cid':elli_id, 'val':1, 'sr':true, 'sel':(vars['chickens'] == 0), 't2':a[a.length - 1]['desc']
			});
			if (aff[elli_id] >= _DREAM_EVENT_MIN && !flags['elli_dream']) {  //DREAM
				a.push({'desc':"DREAM", 'cid':[elli_id, 'f_elli_dream'], 'val':[_DREAM_EVENT_AFF, 1]});
			}
			if (aff[elli_id] >= _ANKLE_EVENT_MIN) {
			...
			a.push({'desc':"MusBox Fix", 'cid':['f_new_mus_box', 'f_old_mus_box'], 'val':[1, -1], 'sr':true, 'sel':(aff[rick_id] >= _RICK_FIX_MIN - 3)});
