function actions_photos_spr_y1(a = [], d = 3, g = 300, is_sunny = 1) {
	...
	*/
	...
	if (is_festival(d)) {
		...
		if (vars['chickens'] > 0 && flags['new_chick'] <= 1) {
			...
			if (flags['new_chick'] == 1) {
				a.push({'desc':"New Chick", 'iid':chicken_id, 'cid':["v_new_chicken_days", "f_new_chick"], 'val':[d + _CHICK_GROW_SLEEPS, -1],
					'sr':(vars['new_chicken_days'].includes(d) && vars['potato_waters'] >= 6), 't0':"Incubate"
				});
			}
			a.push({'desc':"Incubate", 'cid':"f_new_chick", 'val':(_CHICK_BORN_SLEEPS + 1), 'iid':chicken_id,
				'sr':(flags['new_chick'] == 1 || (vars['new_chicken_days'].includes(d) && vars['potato_waters'] >= 6))});
		...
		if (vars['potatoes'] > 0 && (vars['chickens'] > 1 || dow == "THURS")) {
		...
		if ((vars['chickens'] == 0 && g < 1500) || (dow == "MON" && is_sunny)) {
			...
			if (dow == "MON" && is_sunny) {
				...
				a.push({'desc':"Talk (MTN / CHUR)", 'cid':maria_id, 'val':1, 'sr':(aff[maria_id] == 0),
						'sel':(aff[rick_id] < (_RICK_FIX_MIN - 3) || flags['new_mus_box'] == 0)});
			...
			// Get Horse
			a.push({'desc':'Get Horse', 'cid':'f_horse', 'val':1, 'iid':get_npc_id('horse'), 'imp':true});
		...
		} else if (vars['chickens'] > 1 && dow != "THURS" && vars['potatoes'] > 0) {
		...
		if (dow != "MON") {
			...
			a.push({'desc':"Talk ", 'cid':maria_id, 'val':1, 'sr':(aff[maria_id] == 0),
				'sel':((aff[rick_id] < _RICK_FIX_MIN - 3) || ((!is_sunny) || (["SAT", "SUN", "WED"].includes(dow))))
			});
			if (aff[rick_id] >= _RICK_FIX_MIN - 3) {
				a[a.length - 1]['t2'] = "MusBox";
				a.push({'desc':"MusBox", 'cid':[maria_id, 'f_new_mus_box'], 'val':[_MUS_BOX_AFF, -1], 'sr':true,
		...
		} else if (vars['potato_waters'] < 6 && is_sunny && vars['openers'] > 0 && get_horse) {
	...
	if (!is_festival(d) || d == 23) {
		...
		a.push({'desc':"Whistle / Pick up Dog", 'cid':get_npc_id('dog'), 'val':2, 'sr':(!flags['dog_inside'] && aff[get_npc_id('karen')] > 50)});

		if (flags['horse'] == 1) {
			var horse_id = get_npc_id('horse');
			a.push({'desc':"Whistle Horse", 'val':1, 'cid':get_npc_id('horse')});
			// TODO if horse is big, ride, else, talk to
			// TODO: horse values
		}
	}
	return a;
