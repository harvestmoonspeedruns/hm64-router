function actions_photos_spr_y1(a = [], d = 3, g = 300, is_sunny = 1) {
	...
	*/
	...
	var get_horse = (vars['chickens'] == 0 && g >= 1500 && dow != "THURS" && is_sunny == 1);
	...
	if (d == 18) { a.push({'desc':"Ignore Cliff on the Farm", 'iid':cliff_id}); }
	...
	if (vars['chickens'] > 0 && (is_sunny != flags['chicken_outside'] || is_sunny == 0)) {
		has_opener = true;
		if (is_sunny == 0) {
			if (vars['feed'] == 0 && dow != "THURS" && vars['gold'] > 0) {
				...
				if (tmp_seeds > 0) {
					a.push({'desc':"Buy " + tmp_seeds + " Feed", 'cid':['v_gold', 'v_feed'], 'val':[10 * tmp_seeds, tmp_seeds], 'iid':get_npc_id('chicken')});
				}
			}
			if (vars['feed'] > 0 || (dow != "THURS" && vars['gold'] > 0)) {
				a.push({'desc':"Feed Chicken", 'cid':'v_feed', 'val':-1, 'iid':get_npc_id('chicken'),
						'sr':(vars['feed'] == 0 && dow != "THURS" && vars['gold'] > 0)
				});
			}
		}

		// If today is a festival day, these things wont appear normally
		// and youre only doing them because you have to go outside
		// anyway to deal with the chickens
		if (is_festival(d) && flags['chicken_inside'] == 1) {
			// Horse
		...
		if (is_sunny != flags['chicken_outside']) {
			a.push({'desc':("Bring Chicken " + ((is_sunny == 0) ? "In" : "Out") + "side"),
					'val':((is_sunny == 1) ? 1 : -1), 'cid':'f_chicken_outside', 'iid':get_npc_id('chicken'), 'imp':true});
		}
	...
	if (is_festival(d)) {
		...
		if (vars['potato_waters'] < _POTATO_GROW_DAYS && is_sunny && vars['openers'] > 0 && !get_horse) {
			...
			if (sell_stuff) { a.push({'desc':"Sell Stuff"}); }
			a.push({'desc':"Water Potatoes", 'cid':['v_potato_waters', 'v_watering_can_fill'], 'val':[1, -10], 'sr':(!sell_stuff)});
			if (sell_stuff) { sell_stuff = false; }
		}
		if (sell_stuff) { a.push({'desc':"Sell Stuff"}); }
		...
		if (vars['openers'] >= 2) {
			// Music Box Dig
			if (!flags['old_mus_box'] && !get_horse) {
				has_opener = true;
				a.push({'desc':"Equip hoe", 'iid':get_npc_id("musbox")});
		...
		if (dow != "THURS" && vars['potatoes'] > 0 && (vars['chickens'] > 1 || (dow == "WED" && is_sunny == 1))) {
			...
			// ANN
			a.push({'desc':("Talk (" + ((is_sunny == 0) ? "Barn)" : "Ranch) ")), 'cid':ann_id, 'val':1});
		...
		if ((vars['chickens'] == 0 && g < 1500) || (dow == "MON" && is_sunny)) {
			...
			if (flags['berry_strength'] == 0 && vars['chickens'] > 0 && flags['old_mus_box'] == 1 && vars['potato_waters'] >= _POTATO_GROW_DAYS) {
				a.push({'desc':"Wish for Strength", 'cid':'f_berry_strength', 'val':1, 'iid':get_npc_id('goddess'), 'sel':false});
			}
			...
			if (dow == "MON" && is_sunny) {
				...
				// "Egg " <- 1 space
				if (d > 17) {
					a.push({'desc':"Talk (Hot Springs)", 'cid':cliff_id, 'val':2, 'sel':false});
			...
			if (["THURS", "SUN"].includes(dow) && d > 17) {
				a.push({'desc':"Talk (Carp House)", 'cid':cliff_id, 'val':2, 'sel':false});
				a.push({'desc':"Gift    ", 'cid':cliff_id, 'val':4, 't2':"Egg ", 'sr':true, 'sel':false});
				a.push({'desc':"Egg ", 'cid':cliff_id, 'val':8, 't2':"Gift    ", 'sr':true, 'sel':false});
			}
		}
		...
		if (flags['new_mus_box'] == 0 && is_sunny == 1) {
			...
			} else if (dow == "SUN" && aff[rick_id] >= (_RICK_FIX_MIN - 6)) {
	...
	// Feed Dog
	if (vars['chickens'] > 0) {
		a.push({'desc':"Feed Dog", 'cid':get_npc_id('dog'), 'val':1, 'sel':false});
	}
